...
@onready var time_ui = $CanvasLayer/DayNightCycleUI
...
func _ready() -> void:
	...
	canvas_modulate.time_tick.connect(time_ui.set_daytime)
	...
	game_manager.on_money_changed.connect(player_inventory.change_money)
